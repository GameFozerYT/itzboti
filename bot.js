...
client.on('ready', function(){
    ...
    setInterval(function (){
        ...
        i = i+j;
        client.user.setGame(setGame[i],`http://www.twitch.tv/LastReward`);
    }, ms);
...
${prefix}new لفتح التيكت
${prefix}htciket لمعرفة معلومات الكيت
...
client.on("message", message => {
 if (message.content === `${prefix}`) {
  const embed = new Discord.RichEmbed() //by  Texo7 'CODES SERVER'
      .setColor("#000000")//by Texo4777 'CODES SERVER'
      .setDescription(`
${prefix}play ? لتشغيل أغنية برآبط أو بأسم
${prefix}skip ? لتجآوز الأغنية الحآلية
${prefix}pause ? إيقآف الأغنية مؤقتا
...
return channel.send("")
  ...
  if (message.content.toLowerCase().startsWith(prefix + `hticket`)) {
    const embed = new Discord.RichEmbed()
    .setTitle(`:mailbox_with_mail: Texo Help`)
    .setColor(0xCF40FA)
